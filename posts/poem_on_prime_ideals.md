...
By Fundamental theorem of arithmetic (i.e. prime factorization) $a = \prod q_i^\lbrace a_i\rbrace, b = \prod q_j^\lbrace b_j\rbrace$, where $q_i$'s are primes.

But since, $\prod q_i^\lbrace a_i+b_j\rbrace=ab=kp$ and $p$ is a prime, we have $p=q_p^\lbrace a_p+b_p\rbrace$ with $a_p+b_p\ge 1$ and in particular $a_p=0$ since $(p,a)=1$. Thus, $p=q_p^\lbrace b_p\rbrace$ with $b_p\ge 1$, hence $p$ devides $q$.
...
$x = \Sigma^k_\lbrace i=1\rbrace r_i a s_i  b= \Sigma^k_\lbrace i=1\rbrace r_i s_i a b= \Sigma^k_\lbrace i=1\rbrace r_i a s_ikp= p(\Sigma^k_\lbrace i=1\rbrace r_i a s_ik)\in (p)$. $\square$
