...
I.e., $S/\sim$ is a set containing all the equivalence classes; thus the elements of $S/\sim$ are $[a]$, where $[a]$ denotes an equivalence class represented by $a \in S$, and if you prefer the symbolic description, we can write =$S/{\sim}=\lbrace [a]{\sim}\space \mid \space a\in S \text{and} [a]_{\sim}=\lbrace x\in S\space \mid \space x\sim a\rbrace$.<br>
